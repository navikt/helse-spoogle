...
internal sealed class BrukerException(
    message: String,
) : Exception(message) {
    internal abstract val httpStatusCode: HttpStatusCode

    internal class ManglerEpostadresse : BrukerException("Token mangler epostadresse (claim: preferred_username)") {
        override val httpStatusCode: HttpStatusCode = HttpStatusCode.BadRequest
    }

    internal class ManglerNavn : BrukerException("Token mangler navn (claim: name)") {
        override val httpStatusCode: HttpStatusCode = HttpStatusCode.BadRequest
    }

    internal class ManglerIdent : BrukerException("Token mangler ident (claim: NAVident)") {
        override val httpStatusCode: HttpStatusCode = HttpStatusCode.BadRequest
    }

    internal class ManglerOid : BrukerException("Token mangler oid (claim: oid)") {
        override val httpStatusCode: HttpStatusCode = HttpStatusCode.BadRequest
    }
}
